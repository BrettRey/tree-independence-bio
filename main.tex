...
From the Open Tree of Life \citep{hinchliff2015synthesis} I extracted 12 phylogenetic subtrees by querying named clades via the OTL API and exporting induced subtrees in Newick format. Where the synthetic tree records unresolved relationships, the downloaded Newick files contain multifurcations; these are the polytomies discussed in Section~2.2. Tree sizes range from $n = 46$ (Homininae) to $n = 30{,}310$ (Aves).
...
Does the near-miss ratio of biological trees differ from what random trees of the same size would produce? To test this, I generated 200--1{,}000 random labelled trees (uniform via Prüfer sequence) matched to each biological tree size up to $n = 4{,}229$, covering 22 of the 27 specimens. (The five largest trees, $n \ge 6{,}378$, were omitted because computing independence polynomials for hundreds of replicates at those sizes is computationally prohibitive.) Figure~\ref{fig:nullnm} shows the null distributions alongside the observed biological values.

Only 6 of the 22 biological values fall within the null interquartile range, so biological trees are distinguishable from random trees of the same size on this measure. The displacement is systematic: phylogenies sit above the null median (8 of 11), while neuronal arbors fall below it (9 of 11). Polytomy-rich phylogenies, whose high-degree internal nodes arise from unresolved clade structure, have higher~$\nm$ than random labelled trees at the same~$n$; neuronal arbors, whose branching is predominantly binary, fall below.

Despite these departures, the overall convergence toward~1 is shared by biological and random trees alike: both follow a $1 - C/n$ trajectory (Section~\ref{sec:nearmiss}), and the absolute deviations shrink with~$n$. Tree size remains the dominant driver of~$\nm$; branching topology introduces a secondary, systematic shift.
...
\begin{figure}[htbp]
  ...
  \caption{Near-miss ratio for the 22 biological trees with $n \le 4{,}229$ (coloured points) versus null distributions from uniform random trees of the same size (box plots). Blue circles: neuronal arbors; red triangles: phylogenies. Most biological values fall outside the null interquartile range (6/22 inside), with phylogenies above and neurons below the null median, though the overall convergence toward~1 is shared.}
  \label{fig:nullnm}
\end{figure}
...
Mode position and near-miss ratio each capture a different facet of the tree's branching topology, and per-vertex occupation probabilities $\occ(v)$ add a third (Section~3.5). The near-miss ratio correlates strongly with standard tree-shape statistics (Sackin $\rho = 0.924$, Colless-like $\rho = 0.937$), which initially looks promising. A null-model comparison (Section~3.6) shows that tree size is the dominant driver but not the whole story: only 6 of 22 biological values fall within the null interquartile range, with phylogenies systematically above and neurons below the null median.

This suggests a general criterion. A descriptor \term{accommodates} biological structure if its values on real specimens are distinguishable from those on an appropriate null model; it merely \term{redescribes} what the null already predicts if they aren't. The near-miss ratio partially accommodates (it distinguishes tree types) but the biological interpretation of the phylogeny-neuron split remains thin: knowing that polytomy-rich trees have higher~$\nm$ than binary-branching trees at the same size doesn't yet tell a biologist anything actionable. Per-vertex occupation probability is more promising.
...
A full analysis of NeuroMorpho.Org's 260\,000+ reconstructions \citep{ascoli2007neuromorpho} is computationally feasible and would strengthen the empirical base considerably. The null-model result (Section~3.6) shows that biological trees depart systematically from random trees of the same size, but the dominant driver of~$\nm$ is still~$n$. If the independence polynomial captures biologically meaningful structure, it's more likely through the per-vertex probabilities than through the global near-miss ratio.
...
All source data (SWC and Newick input files), computed independence polynomials, and analysis scripts are available at \url{https://github.com/brettrey/tree-indpoly}. NeuroMorpho.Org accession identifiers and Open Tree of Life clade queries are listed in Table~\ref{tab:results}.

\section*{Acknowledgements}

This paper was drafted with the assistance of Claude (Anthropic). All content has been reviewed and revised by the author, who takes full responsibility for the final text (and for any remaining stubbornness in the prose).
